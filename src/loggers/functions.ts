 ...
 * @returns StackLogger
 */
export async function prepare_complete_logger( directory: string ) {
    ...
    return new StackLogger([file_logger, console_logger, operations_logger, cycle_logger]);
}
 ...
 * @returns StackLogger
 */
export async function prepare_test_logger() {
    const file_logger = new FileLogger( '/tmp/test_log.txt' );
    const console_logger = new ConsoleLogger();
    const operations_logger = new OperationsLogger();
    return new StackLogger([file_logger, console_logger, operations_logger]);
}
