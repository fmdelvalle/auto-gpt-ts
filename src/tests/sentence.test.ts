import { createSentencizer } from "../processing/sentencizer";
import { loadEnv } from "../cli/loadenv";
import { get_text_summary } from "../commands/web_requests";
import { buildTestAgent } from "../core/agent";
import { ICommandContext } from "../types";
import { split_text } from "../processing/text";
...
describe('testStrings', () => {
    ...
    it('should correctly split sentences', async () => {
        const sentencizer = createSentencizer('test');
    ...
    it('should group sentences fast', async () => {
        ...
        for await( const chunk of split_text(sentence, ctx.browser.browse_chunk_max_length, ctx.llm.models.fast_llm_model, ctx.llm.embeddings.tokenizer, question) ) {
    ...
    it('should summarize some Wikipedia article', async () => {
        ...
        const summary = await get_text_summary(ctx, { url, question });
