...
import { IAgent, IChatMessage, IRunParameters } from '../types';
import { CreateChatCompletionResponse, CreateEmbeddingRequest, OpenAIApi } from 'openai';
...
: Promise<string> {
  ...
  for (const plugin of agent.config.plugins) {
    if (
      plugin.can_handle_chat_completion({
        messages,
        model,
        temperature,
        max_tokens,
      ...
      const message = plugin.handle_chat_completion({
        messages,
        model,
        temperature,
        max_tokens,
  ...
  for (const plugin of agent.config.plugins) {
    if (plugin.can_handle_on_response()) {
      responsetxt = plugin.on_response(responsetxt);
    }
  }
  return responsetxt;
}
