...
export default class StackLogger implements ILogger {
    ...
    start_spinner (text: string) {
      this.loggers.forEach(logger => logger.start_spinner && logger.start_spinner(text));
    }

    update_spinner (text: string) {
      this.loggers.forEach(logger => logger.update_spinner && logger.update_spinner(text));
    }

    stop_spinner () {
      this.loggers.forEach(logger => logger.stop_spinner && logger.stop_spinner());
    }
  }
